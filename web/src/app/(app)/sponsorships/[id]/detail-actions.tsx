...
export function DetailActions({ id, brandName }: { id: string; brandName: string }) {
  ...
  const remove = async () => {
    if (!confirm(`${brandName} 협찬을 삭제할까요?`)) return;
    const supabase = createClient();
    // RLS guarantees only workspace members can delete.
  ...
  return (
    <div className="flex gap-2 mt-4">
      <Link href={`/sponsorships/${id}/edit`} className="btn btn-primary flex-1 text-center">
        수정하기
      </Link>
